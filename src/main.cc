...
int main(int argc, char **argv)
{
    ...
    if(argc!=4)
    {
        cerr << "Usage: rosrun torosbag torosbag <path to  directory including image file and imu file>   <path to bag>  <the numbers of camera>" << endl;
        return 0;
    }
    ros::start();
  ...
  while(true)
    {
           ...
           for (size_t i = 0; i < numCameras; ++i)
           {
                 cv::Mat image = cv::imread(  path + "/cam" + std::to_string(i)  +"/data/" +*cam_iterators.at(i),  cv::IMREAD_GRAYSCALE);
